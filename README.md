...
| Bracket-delimited text commands (e.g. `[[pbas200]]`) | `EmbeddedCmd.c` | Ported for `pbas`/`pbar`/`pmod`/`pmor`/`volm`/`volr`/`emph`/`cmnt`/`vers`/`dlim`/`slnc`/`rset`/`sync` (`lintalker/_embeddedcmd.py`'s `scan_bracket_commands`; `slnc` inserts a real silence at the exact word position, `rset`/`sync` recognize and queue commands that are genuine no-ops/documented stubs in `do_ctrl` too, the rest apply at clause start); `rate`/`xtnd`/`char`/`mode`/`nmbr` not ported; not verifiable frame-exact against the compiled `lintalker-c` test harness — see `docs/architecture.md` |
...
(`[[sync12345]]`, a genuine no-op in the reference too) — pitch/mod/
volume are applied at the start of the clause they appear in, not the
exact phoneme position; see `docs/architecture.md`. Remaining known
gaps: no number/abbreviation expansion, no
`rate`/`xtnd`/`char`/`mode`/`nmbr` embedded commands, and
`Zap_POS` (unreachable) — see `docs/architecture.md` for
