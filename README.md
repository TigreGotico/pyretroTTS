...
| Bracket-delimited text commands (e.g. `[[pbas200]]`) | `EmbeddedCmd.c` | Ported for `pbas`/`pbar`/`pmod`/`pmor`/`volm`/`volr`/`emph`/`cmnt`/`vers`/`dlim`/`slnc`/`rset`/`sync`/`xtnd wpos`/`rate`/`ratr`/`nmbr`/`mode`/`char` (`lintalker/_embeddedcmd.py`'s `scan_bracket_commands`; `slnc`/`rate`/`xtnd wpos`/`nmbr`/`char` apply at the exact word position, the rest apply at clause start; `rset`/`sync` recognize and queue commands that are genuine no-ops/documented stubs in `do_ctrl` too; `mode PHON` parses raw phoneme mnemonics via `lintalker/_rawphon.py`'s bit-exact `MAGIC_MAP`; `char LTRL` spells words letter-by-letter via `lintalker/_letters.py`'s bit-exact-per-letter `LETTER_PHONEMES`); not verifiable frame-exact against the compiled `lintalker-c` test harness — see `docs/architecture.md` |
...
"3:05" -> "three oh five", "3:00" -> "three o'clock"). The `mode`
embedded command's `PHON` setting (`[[mode PHON]]...[[mode TEXT]]`)
parses raw phoneme mnemonics directly (e.g. `_1AAt` -> the phonemes
for "at") via a bit-exact transcription of the reference's own
mnemonic table. The `char` embedded command's `LTRL` setting
(`[[char LTRL]]...[[char NORM]]`) spells words letter-by-letter (e.g.
"cab" -> "see ay bee") using pronunciations extracted bit-exact,
letter-by-letter, from the compiled C reference — see
`docs/architecture.md` for
specifics and a documented multi-letter verification caveat. You can
still synthesize from an
