...
| Bracket-delimited text commands (e.g. `[[pbas200]]`) | `EmbeddedCmd.c` | Ported for `pbas`/`pbar`/`pmod`/`pmor`/`volm`/`volr`/`emph`/`cmnt`/`vers`/`dlim`/`slnc`/`rset`/`sync`/`xtnd wpos` (`lintalker/_embeddedcmd.py`'s `scan_bracket_commands`; `slnc` inserts a real silence and `xtnd wpos` sets a real POS override, both at the exact word position; `rset`/`sync` recognize and queue commands that are genuine no-ops/documented stubs in `do_ctrl` too; the rest apply at clause start); `rate`/`char`/`mode`/`nmbr` not ported; not verifiable frame-exact against the compiled `lintalker-c` test harness — see `docs/architecture.md` |
...
(`[[sync12345]]`, a genuine no-op in the reference too), and word
...
expansion, no `rate`/`char`/`mode`/`nmbr` embedded commands, and
