...
| Bracket-delimited text commands (e.g. `[[pbas200]]`) | `EmbeddedCmd.c` | Ported for `pbas`/`pbar`/`pmod`/`pmor`/`volm`/`volr`/`emph`/`cmnt`/`vers`/`dlim`/`slnc`/`rset` (`lintalker/_embeddedcmd.py`'s `scan_bracket_commands`; `slnc` inserts a real silence at the exact word position, `rset` recognizes and queues a `C_reset` `do_ctrl` already stubs, the rest apply at clause start); `rate`/`xtnd`/`char`/`mode`/`nmbr`/`sync` not ported; not verifiable frame-exact against the compiled `lintalker-c` test harness — see `docs/architecture.md` |
...
applied, matching `do_ctrl`'s existing documented stub) — pitch/mod/
...
`rate`/`xtnd`/`char`/`mode`/`nmbr`/`sync` embedded commands, and
