...
| Bracket-delimited text commands (e.g. `[[pbas200]]`) | `EmbeddedCmd.c` | Ported for `pbas`/`pbar`/`pmod`/`pmor`/`volm`/`volr`/`emph`/`cmnt`/`vers`/`dlim`/`slnc`/`rset`/`sync`/`xtnd wpos`/`rate`/`ratr`/`nmbr`/`mode` (`lintalker/_embeddedcmd.py`'s `scan_bracket_commands`; `slnc`/`rate`/`xtnd wpos`/`nmbr` apply at the exact word position, the rest apply at clause start; `rset`/`sync` recognize and queue commands that are genuine no-ops/documented stubs in `do_ctrl` too; `mode PHON` parses raw phoneme mnemonics via `lintalker/_rawphon.py`'s bit-exact `MAGIC_MAP`); `char` not ported (needs the 26 letter-name pronunciations from the `Symbols` dictionary, not yet extracted bit-exact); not verifiable frame-exact against the compiled `lintalker-c` test harness — see `docs/architecture.md` |
...
mnemonic table. Remaining known gap: the `char` embedded command
(letter-by-letter spelling, e.g. "B" -> "bee") — see
