...
| Bracket-delimited text commands (e.g. `[[pbas200]]`) | `EmbeddedCmd.c` | Ported for `pbas`/`pbar`/`pmod`/`pmor`/`volm`/`volr`/`emph`/`cmnt`/`vers`/`dlim` (`lintalker/_embeddedcmd.py`'s `scan_bracket_commands`, applied at clause start rather than the exact phoneme position); `rate`/`rset`/`xtnd`/`char`/`mode`/`nmbr`/`slnc`/`sync` not ported; not verifiable frame-exact against the compiled `lintalker-c` test harness — see `docs/architecture.md` |
...
changes (applied at the start of the clause they appear in, not the
exact phoneme position — see `docs/architecture.md`). Remaining known
gaps: no true compound-noun splitting, no number/abbreviation
expansion, no `rate`/`rset`/`vers`/`xtnd`/`char`/`cmnt`/`dlim`/`mode`/
`nmbr`/`emph`/`slnc`/`sync` embedded commands — see
