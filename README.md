...
| Bracket-delimited text commands (e.g. `[[pbas200]]`) | `EmbeddedCmd.c` | Ported for `pbas`/`pbar`/`pmod`/`pmor`/`volm`/`volr` (`lintalker/_embeddedcmd.py`'s `scan_bracket_commands`, applied at clause start rather than the exact phoneme position); `rate` and the remaining ~11 commands not ported; not verifiable frame-exact against the compiled `lintalker-c` test harness — see `docs/architecture.md` |
