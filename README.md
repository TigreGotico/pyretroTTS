...
| Bracket-delimited text commands (e.g. `[[pbas200]]`) | `EmbeddedCmd.c` | Ported for `pbas`/`pbar`/`pmod`/`pmor`/`volm`/`volr`/`emph`/`cmnt`/`vers`/`dlim`/`slnc` (`lintalker/_embeddedcmd.py`'s `scan_bracket_commands`; `slnc` inserts a real silence at the exact word position, the rest apply at clause start); `rate`/`rset`/`xtnd`/`char`/`mode`/`nmbr`/`sync` not ported; not verifiable frame-exact against the compiled `lintalker-c` test harness — see `docs/architecture.md` |
...
version checks (`[[vers...]]`), and changing the command delimiters
themselves (`[[dlim...]]`) — pitch/mod/volume are applied at the start
of the clause they appear in, not the exact phoneme position; see
`docs/architecture.md`. Remaining known gaps: no number/abbreviation
expansion, no `rate`/`rset`/`xtnd`/`char`/`mode`/`nmbr`/`sync` embedded
