...
# 159 "/usr/include/bits/sched.h" 3
...
# 26 "/usr/include/bits/setjmp.h" 3
# 1 "/usr/include/bits/wordsize.h" 1 3
...
# 89 "../include/kernel.h"
# 1 "../../../dectalkf.h" 1
# 90 "../include/kernel.h" 2

# 1 "../include/port.h" 1
# 92 "../include/kernel.h" 2
...
typedef struct queue_semaphore {
 struct queue_semaphore *head;
 struct queue_semaphore *tail;
 PCB * process;
} QUEUE_SEMAPHORE;
...
# 405 "../include/kernel.h"
struct dgt_packet {
 struct dgt_packet *link;
 unsigned int high_addr;
 unsigned int low_addr;
 unsigned int length;
 unsigned int data[(65*8)];
};
# 449 "../include/kernel.h"
struct dtpc_code_pages {
 struct dtpc_code_pages *link;
 int dos_id;
 unsigned char *translation_page;

};

struct dtpc_language_tables {
 struct dtpc_language_tables *link;
 ...
 unsigned char * lang_ascky;
 int lang_ascky_size;
 unsigned int * lang_reverse_ascky;
 unsigned char * lang_arpabet;
 int lang_arpa_size;
 int lang_arpa_case;
 const unsigned char * * lang_typing;
 const unsigned char * * lang_error;
};
...
struct share_data {
# 560 "../include/kernel.h"
 S32 *fdic_index[7];
 unsigned char *fdic_data[7];
 S32 fdic_entries[7];
 S32 fdic_bytes[7];
 S32 *fdic_fc_entry[7];
 S32 fdic_fc_entries[7];

 S32 *udic_index[7];
 unsigned char *udic_data[7];
 S32 udic_entries[7];
 S32 udic_bytes[7];

 S32 *adic_index[7];
 unsigned char *adic_data[7];
 S32 adic_entries[7];
 S32 adic_bytes[7];

 S32 *foreigndic_index[7];
 unsigned char *foreigndic_data[7];
 S32 foreigndic_entries[7];
 S32 foreigndic_bytes[7];

 DT_HANDLE fdicMapObject[7];
 DT_HANDLE fdicFileHandle[7];
 LPVOID fdicMapStartAddr[7];

 DT_HANDLE foreigndicMapObject[7];
 DT_HANDLE foreigndicFileHandle[7];
 LPVOID foreigndicMapStartAddr[7];
 ...
 DT_SEMAPHORE print_sem;
 RING * in_ring;
 RING * out_ring;
 int eight_bit;

 DT_SEMAPHORE isa_sem;
 DT_SEMAPHORE flush_sem;
 unsigned int isa_status;
# 613 "../include/kernel.h"
 P_PIPE cmd_pipe;
 ...
 P_PIPE sync_pipe;
 P_PIPE buffer_pipe;
# 627 "../include/kernel.h"
 P_PIPE buffer_delay_pipe;
# 642 "../include/kernel.h"
 int lang_curr;
 int lang_ready[7];
 ...
 struct spc_packet * spc_pkt_save;





 LPCRITICAL_SECTION pcsSpcPktSave;



 QUEUE_SEMAPHORE index_pending;
 QUEUE_SEMAPHORE index_active;
 ...
 int spc_flush_type;
 QUEUE_SEMAPHORE spc_pool;
 QUEUE_SEMAPHORE spc_active;
 QUEUE_SEMAPHORE dgt_pool;
 QUEUE_SEMAPHORE dgt_active;

 DT_SEMAPHORE spc_sync;
 DT_SEMAPHORE spc_resume;
 DT_SEMAPHORE spc_dma;
 DT_SEMAPHORE text_sync;
# 709 "../include/kernel.h"
 unsigned int modeflag;
 unsigned int logflag;
 unsigned int sayflag;
 unsigned int pronflag;
 unsigned int phoneme_mode;
 unsigned int wbreak;
 unsigned int input_timeout;
 int volume;
 int vol_att;
# 764 "../include/kernel.h"
 short last_voice;
 short sprate;
 short uiCurrentSpeaker;
 DWORD dwLastPhoneme;
 U32 uiSampleRate;
 double SamplePeriod;
 ...
 int pitch_delta;
# 804 "../include/kernel.h"
 unsigned char iso_to_upper[256];
 unsigned char iso_to_lower[256];





 struct dtpc_language_tables *loaded_languages;
 unsigned char * ascky;
 int ascky_size;
 unsigned int * reverse_ascky;
 unsigned char * arpabet;
 int arpa_size;
 int arpa_case;
 const unsigned char * * typing_table;
 const unsigned char * * error_table;
# 830 "../include/kernel.h"
  unsigned short debug_switch;




  unsigned short gender_switch;
# 853 "../include/kernel.h"
  FILE *dbglog;
# 864 "../include/kernel.h"
  BOOL bInTypingMode;
...
typedef struct share_data KSD_T;
typedef KSD_T *PKSD_T;
typedef KSD_T **PPKSD_T;
# 1066 "../include/kernel.h"
 RING * create_ring(int);
int put_ring( RING *,unsigned char *);
int get_ring( RING *,unsigned char *);
void flush_ring( RING *);
unsigned int test_ring( RING *);
void lock_ring( RING *);
void unlock_ring( RING *);
# 1132 "../include/kernel.h"
struct share_data *get_share();
# 113 "ph_defs.h" 2
# 1 "../include/dectalk.h" 1
# 105 "../include/dectalk.h"
typedef char FLAG;
# 118 "../include/dectalk.h"
typedef struct SEQ {
 int s_type;
 char s_badf;
 char s_pintro;
...
# 1 "../include/samprate.h" 1
# 118 "ph_defs.h" 2
# 128 "ph_defs.h"
# 1 "p_all_ph.h" 1
# 41 "p_all_ph.h"
# 1 "../include/l_com_ph.h" 1
# 42 "p_all_ph.h" 2
# 1 "../include/l_all_ph.h" 1
# 40 "../include/l_all_ph.h"
# 1 "../include/l_com_ph.h" 1
# 41 "../include/l_all_ph.h" 2
# 43 "p_all_ph.h" 2
# 129 "ph_defs.h" 2
# 586 "ph_defs.h"
...
IQUEUE;
# 58 "ph_def.h" 2
# 1 "../include/dt_abort.h" 1
# 50 "../include/dt_abort.h"
...
struct TTS_HANDLE_TAG
{
  ...
  unsigned long hWnd;
  unsigned long hTextToSpeechWnd;

  PKSD_T pKernelShareData;
  PVOID pCMDThreadData ;
  PVOID pLTSThreadData ;
  PVOID pVTMThreadData ;
  PVOID pPHThreadData ;
# 269 "../api/tts.h"
  HEVENT_T hMallocSuccessEvent;
  ...
  UINT uiID_Bookmark_Message;
  UINT uiID_Wordpos_Message;
  UINT uiID_Start_Message;
  UINT uiID_Stop_Message;
  UINT uiID_Visual_Message;
...
# 348 "../api/tts.h"
  ...
  DWORD dwTTSInstanceParameter;
# 367 "../api/tts.h"
  HMUTEX_T pcsCallback;
...
# 409 "../api/tts.h"
  BOOL bInReset;
  char *szPhonemeBuffer;

  DWORD dwPhonemeBufferSize;
  DWORD dwPhonemeBufferPtr;
  DWORD dwPhonemeCvtFlags;
...
typedef struct SPDEFS_TAG {
 ...
 short breathiness;
 short richness;
 short num_fixed_samp_og;
 short laryngealization;
 short head_size;
 short formant4_res_freq;
 short formant4_bandwidth;
 short formant5_res_freq;
 short formant5_bandwidth;
 short parallel4_freq;
 short parallel5_freq;
 short gain_frication;
 short gain_aspiration;
 short gain_voicing;
 short gain_nasalization;
 short gain_cfr1;
 short gain_cfr2;
 short gain_cfr3;
 short gain_cfr4;
...
# 1 "ph_data.h" 1
# 102 "ph_data.h"
extern const short paul_8[39];
extern const short betty_8[39];
extern const short harry_8[39];
extern const short frank_8[39];
extern const short kit_8[39];
extern const short ursula_8[39];
extern const short rita_8[39];
extern const short wendy_8[39];
extern const short dennis_8[39];

extern const short paul[39];
extern const short betty[39];
extern const short harry[39];
extern const short frank[39];
extern const short kit[39];
extern const short ursula[39];
extern const short rita[39];
extern const short wendy[39];
extern const short dennis[39];

extern const short gr_paul[39];
extern const short gr_betty[39];
extern const short gr_harry[39];
extern const short gr_frank[39];
extern const short gr_kit[39];
extern const short gr_ursula[39];
extern const short gr_rita[39];
extern const short gr_wendy[39];
extern const short gr_dennis[39];
# 270 "ph_data.h"
...
typedef struct DPH_TAG
{
 ...
 short addjit;
 short sprate;
 short allophons[300 + 8 + 2];
 U32 allofeats[300 + 8 + 2];
 short alloopenq[300 + 8 + 2];
 short allodurs[300 + 8 + 2];
 unsigned long wordclass[300 + 8 + 2];
 ...
 short perpause;
 short f0tar[300 + 8 + 2];
 short f0type[300 + 8 + 2];
 short f0length[300 + 8 + 2];
 short Cntfromlast;
 short keepallo;
 short lastallo;
 short keepdur;
...
# 687 "ph_data.h"
 ...
 short vowel_portion;
 short test_targf0;
 short targf0_increment;
 short diff_targf0;
 short impulse_width_increment;
 short diff_impulse_width;
 short ramp_targf0;
 short ramp_impulse_width;
 short ramp_delayf0;
 short last_preamble_command;

  short hat_seen, wordcount;
  short Word_has_stress;
...
extern int spcwrite (PKSD_T pKsd_t, unsigned short * spc_data);
...
extern void logitem (LPTTS_HANDLE_T phTTS, short *buf);
...
# 152 "ph_prot.h"
void kltask (LPTTS_HANDLE_T phTTS);
...
short F_Nb_Syll (PDPH_T pDph_t, short nphon) {
  ...
  do {
    if ((all_featb[(pDph_t->allophons [i])>>8][(pDph_t->allophons [i])&0x00ff]) & 0000001) NbS++;
    i++;
  } while ( !(pDph_t->allofeats[i] & 0x0001) && i<(pDph_t->nallotot));
...
void fr_phinton (LPTTS_HANDLE_T phTTS) {
  ...
  short f0_mphrase_position [6] = {30, 0, 65, 0, 50, 45};
  ...
  FLAG Mot_Accentue;
  FLAG Dernier_Mot_Acc;
  FLAG Raise;
  FLAG Fin_Groupe;
  short Syll_Mot;
  short Syll_Synt;
  short Syll_Mot_Restantes;
  ...
  for (nphon = 0; nphon < pDph_t->nallotot; nphon++) {
    ...
    if (struccur & 0x0001) {
      ...
      if (struccur & 0x0002) {
        ...
        Mot_Accentue = (struccur & 0x0008) >> 3;
        Fin_Groupe = (struccur & 0x0010) >> 4;
        Dernier_Mot_Acc = (struccur & 0x0020) >> 5;
        Raise = (struccur & 0x0040) >> 6;
    ...
    if ((pDph_t->f0mode == 5) ||
      ...
      if (pDph_t->user_f0 [nphon] != 0) {
        ...
        make_f0_command_fr (phTTS, 0, 0, (short)(2000 + pDph_t->user_f0 [nphon]),
                         0, 0, &cumdur, nphon);
      }

    } else if ((pDph_t->f0mode == 1) ||
               (pDph_t->f0mode == 3)) {
      if ( (feacur & 0000001) && Mot_Accentue) {
        ...
        if (pDph_t->f0mode == 3) {
          ...
          targf0 = (((S32)(pDph_t->arg1) * (S32)(targf0)) / (S32)(32));
          make_f0_command_fr (phTTS, 1, 2, targf0, delayf0, 20, &cumdur, nphon);
          ...
          targf0 = f0_stress_level [01] +
                   f0_mphrase_position [pDphsettar->nrises_sofar];
          regle (phocur, "Last vowel of a stressed word");
          if (pDphsettar->nrises_sofar < 6 - 1) pDphsettar->nrises_sofar++;
        } else {

          targf0 = f0_stress_level [02];
          regle (phocur, "Stressed word, but not final vowel of the word");
        }

          pDph_t->arg1 = pDph_t->scale_str_rise;
          targf0 = (((S32)(pDph_t->arg1) * (S32)(targf0)) / (S32)(32));
          make_f0_command_fr (phTTS, 1, 2, targf0, delayf0, 10, &cumdur, nphon);

        }



        if ((Syll_Synt_Restantes == 1) && (pDph_t->hatpos == 2)) {
          if (pDph_t->f0mode == 1) {
# 274 "ph_fr_inton2.c"
            if ((pDph_t->allofeats [pDph_t->nallotot-1]) & 0x0004) {
              f0fall = 212;
...
# 295 "ph_fr_inton2.c"
      ...
      } else if (feacur & 0000001) {
        delayf0 = pDph_t->allodurs [nphon] >> 2;
        targf0 = f0_stress_level [0];
        make_f0_command_fr (phTTS, 1, 2, targf0, delayf0, 20, &cumdur, nphon);
    ...
    if ( ( nphon <= (pDph_t->nallotot-1) &&
            (nphon > 0 && (pDph_t->allophons[nphon] & 0x00FF) != 0) )
 || nphon==0 )
...
# 1 "ph_inton0.c" 1
# 1296 "ph_inton0.c"
extern short us_f0_stress_level[];
extern short us_f0_phrase_position[];
