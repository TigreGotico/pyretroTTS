...
|---|---|---|
| `mt4.h`, `Fsynth.h`, `SpeechEqu.h` | `pylintalker/_consts.py` | Constants, fixed-point math helpers (`mMul2`, `mDiv`, `clamp`, ...) |
| `mt4.h` phoneme enum | `pylintalker/_phonemes.py` | Phoneme ids, names, categories |
| `Data.c` | `pylintalker/_data.py` | Voice definitions, coefficient tables (generated by `tools/extract_data.py`) |
| `Say.c`, `formantSynth.c`, `BackEnd.c` | `pylintalker/_backend.py` | Frame-by-frame formant synthesis: `say_frame`, `e_fill_next_frame`, `head_rules`/`tail_rules`, ramp/pitch handling, `do_note`/`do_note_script` (embedded note-driven pitch) |
| `EngToP.c` | `pylintalker/_engtop.py` | English word -> phoneme dispatch via letter-to-sound rules. Operates on a single pre-isolated, already-uppercased word (see `test/test_engtop.py`). |
| `FrontEnd.c` (tokenizer + per-word dispatch only) | `pylintalker/_frontend.py` | Splits text into words + end-of-word punctuation, calls `_engtop.engtop()` per word. Preserves pure digit runs as their own token (see `_numbers.py`). |
| `FrontEnd.c` (`PartialNumberToPhonemes`/`AppendTwoDigitPhonemes`/`AppendThreeDigitPhonemes`, cardinal-number reading) | `pylintalker/_numbers.py` | Ported, with a documented verification caveat (see "Known gaps") — individual number words bit-exact, the grouping algorithm faithful but not end-to-end verified since the reference's own `Symbols`-dictionary numeric keys are corrupted |
| `BackEnd.c` (`Collect_FE_Tokens` + `Flag_PhonBuf_1`/`MarkSyllable`/`MarkSyllableStart`/`MarkBoundry`, adapted for the unported `FrontEnd.c`/`Morph.c` token stream) | `pylintalker/_assembly.py` | Sentence-level stress/word/punctuation/syllable bookkeeping, verified bit-exact against a real C oracle for four sentences (`test/test_assembly.py`). |
| `BackEnd.c` (`Fill_Phon_Buf_2`), `formantSynth.c` (`Insert_Closure_Release`) | `pylintalker/_phonbuf2.py` | Allophone selection (dark L, R-coloring, t/d-flapping, glottalization, DH-vowelizing, y-slurring, EN/EL, glottal-stop insertion) plus plosive-release insertion, verified bit-exact against the C reference across voices/sentences (`test/test_assembly_pipeline.py`). |
| `BackEnd.c` (`Pitch_RaiseAndFall` + `Count_StressVowels_Till_Boundry`/`Any_StressVowels_Remain`/`Count_Vowels_Till_Boundry`) | `pylintalker/_pitchcontour.py` | Sentence-level pitch-contour ctrl-bit flagging (`kPitchRise`/`kPitchFall`/`kPitchRise1`/`kPitchFall1`), verified bit-exact against the C reference across voices/sentences (`test/test_assembly_pipeline.py`). |
| `BackEnd.c` (`Mod_Duration`) | `pylintalker/_moduration.py` | Per-phoneme duration assignment (stress, syllable position, obstruent voicing, clusters, glides, speaking rate), verified bit-exact against the C reference across voices/sentences (`test/test_assembly_pipeline.py`). |
| `BackEnd.c` (`Fill_Pitch_Buf`, `Store_F0_and_Time`) | `pylintalker/_pitchbuf.py` | Turns the ctrl-bit pitch contour into `pitch_Buf_Freq`/`pitch_Buf_Time`/`pitch_Buf_Flags`, verified bit-exact against the C reference across voices/sentences (`test/test_pitchbuf.py`). |
| `Engine.c` | `pylintalker/_engine.py` | Top-level init/speak/reset/rate/pitch/volume API, built on `_backend.py`. `e_speak_buffer` (the text-in entry point) and a few fsynth-dependent setters (`e_reset_params`, `e_use_voice`, `e_reinit_voice`) raise `NotImplementedError` naming the specific unported upstream C function they need. |
| `BackEnd.c` (`DoCtrl`, the per-phoneme `CMDQueue` dispatcher: absolute/relative pitch, volume, mod) | `pylintalker/_embeddedcmd.py` | Ported (see `test/test_embeddedcmd.py`); `C_reset`/`C_voice` are unimplemented/no-op the same way upstream leaves them, pending `ResetVoice`/`NewVoice` |
| `EmbeddedCmd.c` (the FrontEnd bracket-delimited text-command parser, e.g. `[[pbas200]]` -- `[[`/`]]` are the default delimiters, `mt4.h`'s `defaultCmdBeginDelim`/`defaultCmdEndDelim`, a distinct mechanism from `DoCtrl` above — it sets `PendingCommands` bits that `FrontEnd.c` later turns into `CMDQueue` entries via `QueueCommand`, except `emph`/`xtnd wpos`/`rate` which write directly to per-token/per-word state) | `pylintalker/_embeddedcmd.py`'s `scan_bracket_commands` | Ported for `pbas`/`pbar`/`pmod`/`pmor`/`volm`/`volr`/`rset`/`sync` (applied as an immediate state change at clause start via `do_ctrl`, not true per-phoneme positioning -- `rset` correctly raises `NotImplementedError` via `do_ctrl`'s existing stub, `sync` is a genuine no-op matching `do_ctrl` having no `C_sync` case, same as the real `DoCtrl`), `emph`/`emph-`/`xtnd`'s `wpos` selector/`slnc`/`rate`/`ratr` (all applied at the exact word position -- `emph`/`xtnd wpos` via token fields, `slnc`/`rate` via `sa.note_buf`/`sa.rate_buf` consumed by `_moduration.mod_duration`), `cmnt`/`vers` (no-ops, stripped), and `dlim` (changes the begin/end delimiter used for later commands in the same text), `nmbr` (a latched `NORM`/`LTRL` mode switching digit-string tokens between cardinal grouping and digit-by-digit reading, applied as a running flag in `_assembly.collect_fe_tokens` rather than a one-shot per-word override), `mode` (a latched `TEXT`/`PHON` mode parsing raw phoneme mnemonics via `pylintalker/_rawphon.py`'s bit-exact `MAGIC_MAP`, applied by `scan_bracket_commands` itself rather than deferred to `collect_fe_tokens`), and `char` (a latched `NORM`/`LTRL` mode spelling alphabetic words letter-by-letter via `pylintalker/_letters.py`'s per-letter bit-exact `LETTER_PHONEMES`, applied the same running-flag way as `nmbr`); cannot be verified frame-exact against `lintalker-c`'s compiled `test_harness` -- see "Known gaps" |
| `Morph.c` (`ResolvePOS`, `PlacePhrasing`, `SetPOS_FromSuffix` including `Zap_POS`, `DoMorph`'s suffix functions) | `pylintalker/_morph.py` (suffix decomposition) + `pylintalker/_assembly.py` (`resolve_pos`/`_place_phrasing`) | Every top-level function is ported, including `Zap_POS` (`apply_pos_from_suffix`'s `hasAlt`-true branch, which also selects the root's alternate pronunciation `phon_hold` when the ALT `pos_code2` reading wins -- `PlacePhrasing`'s `inParen`/SEP7 aren't real gaps -- neither is ever exercised by the C reference itself, see "Known gaps"); `Search_Suffix`'s real `SuffixTab` trie data is approximated with an ordered `endswith()` cascade instead of extracted |
| `english_lex.c`/`English.lex` | `pylintalker/_lexicon.py` | Dictionary lookup (`lookup(word)`), verified bit-exact against the real engine for 249 words spanning common/rare/compound-noun/abbreviation entries (`test/test_lexicon.py`). |
| `Sounds.c` | not applicable | Pure data, not logic: raw embedded PCM sample blobs for `kUseSyncSnd` voices (`Bells_Sound`/`Boing_Sound`/`Bubbles_Sound`/`Cellos_Sound`/`Deranged_Sound`/`Hysterical_Sound`/`PipeOrgan_Sound`) plus the `Symbols` dictionary (`Sounds.c:9829`, see `_numbers.py`/`_letters.py`'s docstrings for what's extracted from it and its confirmed narrow corruption). The raw PCM sample BYTES themselves are never consumed by this port's formant synthesis (confirmed: only the marker TIMESTAMPS from each sample's header matter, already extracted into `_data.py`'s `Bells_Markers`/`Hysterical_Markers`, consumed by `_moduration.py`'s `sync_On_Marker` branch -- see the `BackEnd.c`/`_moduration.py` row above); there is no synthesis-relevant logic in this file left to port. |
...
`pylintalker.api.synthesize_text(voice_dict, text)` composes, in
...
- (Fixed, with a documented verification caveat) Number expansion:
  ...
  `pylintalker/_numbers.py`'s module docstring for the exact scope; see
  ...
  CORRECTED: an earlier pass of this doc (and `README.md`) listed
  "cent-only currency" (a bare `¢`, with no `$`) as a remaining gap.
  Grepping every `kAddCent` reference in `FrontEnd.c` (5 total) shows
  there is no such thing in the C reference at all: `kAddCent` is ONLY
  ever set by `kPeriodTok`'s dollar-flagged branch (`FrontEnd.c:2104`,
  already ported above) and propagated from that token to the
  following numeric token (`FrontEnd.c:2064-2066`, functionally the
  same propagation `_cent_out`/`is_cent` already implement via their
  own side-channel instead of a literal `prev_Tok->addFlags` copy).
  There is no independent bare-`¢`-symbol code path anywhere in the
  source to port -- another phantom gap, same class as "ordinal"/
  "phone number" corrected earlier in this file.
...
- (Partially fixed) `EmbeddedCmd.c`'s bracket-delimited text-command
  ...
  (Fixed) `mode` (`Parse_mode_Command`/`ChangeInputMode`'s `TEXT`/
  `PHON` toggle, search `EmbeddedCmd.c` for `Parse_mode_Command`)
  switches the tokenizer into accepting raw phoneme mnemonics instead
  of English words (`kRawPhonemes`, `GetNextPhonemeOpcode`/
  `CollectPhonemeToken`, `FrontEnd.c:247-345`) -- e.g. `[[mode
  PHON]]_1AAt[[mode TEXT]]` speaks the literal phonemes for "at". This
  turned out to be tractable despite initially being documented
  alongside `char` as "a wholly separate input grammar, out of scope":
  the mnemonic-to-opcode mapping (`Data.c:3307`'s `MAGIC_CHAR_MAP[]`/
  `Data.c:3394`'s `MAGIC_OPCODE_MAP[]`) is a literal, compile-time C
  source table -- not a runtime `Symbols`-dictionary lookup -- so
  `pylintalker/_rawphon.py`'s `MAGIC_MAP` is a direct, bit-exact
  transcription of it, the same class of source as `_OH`/`_DOLLAR`/
  `_CENT`/`_CLOCK` in `_numbers.py`, carrying no corruption or
  verification caveat at all.

  `_rawphon.parse_raw_phonemes` ports `GetNextPhonemeOpcode`'s greedy
  2-char-then-1-char scan; `split_into_word_groups` ports
  `CollectPhonemeToken`'s splitting on a `_Word_`/`_EmphWord_` opcode
  (except as the very first opcode collected, matching its `&&
  tokLen` guard). `_embeddedcmd.scan_bracket_commands` applies `mode`
  itself (unlike `nmbr`, which defers its latched state to
  `_assembly.collect_fe_tokens`): while `PHON` mode is active, the
  `_emit_segment` helper routes text between bracketed commands
  through the raw-phoneme parser instead of treating it as literal
  English, synthesizing one `"RAWPHONn"` placeholder word per opcode
  group and recording that group's phonemes in a new
  `raw_phon_overrides: {word_index: phon_str}` return value.
  `_assembly.collect_fe_tokens` gained a matching `raw_phon_overrides`
  parameter: a word at a recorded index gets its `FEWordToken` built
  directly from the recorded phonemes, bypassing `make_fe_word_token`'s
  normal dictionary/`EngToP`/digit-routing entirely on the meaningless
  placeholder string (mirrors `CollectPhonemeToken` producing a
  `kRawPhonemeTok` whose `phonStr` is used as-is, with its default
  `SetPOStoVal(tok, kNoun)`, `FrontEnd.c:334-345`).

  Verified: `test/test_rawphon.py` (the mnemonic table and
  parsing/splitting logic in isolation) and `test/test_embeddedcmd.py`'s
  `test_scan_bracket_commands_mode_phon_parses_raw_phonemes`/
  `test_scan_bracket_commands_mode_phon_multiple_word_groups`/
  `test_scan_bracket_commands_mode_phon_unterminated_runs_to_end`/
  `test_mode_phon_reaches_word_token_end_to_end`/`test_mode_phon_
  applied_end_to_end_via_build_phoneme_plan_does_not_crash` (the last
  one only checks the pipeline runs without crashing -- like every
  other embedded command, this cannot be verified frame-exact against
  ...
  `pylintalker/_letters.py`'s `LETTER_PHONEMES` holds these 26 bit-exact
