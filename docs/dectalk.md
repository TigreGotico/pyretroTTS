...
`ph/` chain above it is ported and composed: `phclause.speak_phonemes`
...
out-of-dictionary words is ported too (`lts_rules.py`); a lone dictionary or
rule-driven word composes text -> phonemes -> PCM sample-exact vs the oracle.

The **multi-word / multi-clause sentence front end** is ported
...
The **word-level syntactic marking** is ported (`grammar_us.py`): the US
...
**Inflectional morphology is ported** (`morph_us.py`): a word that misses the
...
reading** are ported. The single-character word `a` reduces to the schwa
...
render bit-exact. The **`Dr.`/`St.` title-abbreviation disambiguation** is
...
(`title_abbrev_us.py`, below) closes `dr. smith` -- the battery is **56/56
...
`be`-form auxiliary (`are you there?`, `is it cold?`) are closed by the
...
`DECtalkEngine.synthesize(text, voice)` renders plain text through this native
...
**YES -- the general running-text battery is 56/56 text -> PCM sample-exact
...
therefore identical PCM; the borrow in `test_dectalk_endtoend` is a ported
